...
test('İstanbul Ankara 10:00 - 17:00 arası THY Gidiş Dönüş ucuşlar', async ({ page }) => {
  ...
  await page.getByTestId('search-round-trip-label').locator('span').click();
  ...
  await expect(page.locator('#SearchRoot')).toContainText('10:00 ile 17:00 arası');
  await page.locator('#SearchRoot div').filter({ hasText: 'Havayolları' }).nth(4).click();
  await page.locator('span').filter({ hasText: /^Türk Hava Yolları$/ });
